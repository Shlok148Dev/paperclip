...
class PolicyEngine:
    ...
    def evaluate(self, pr: PullRequest) -> Tuple[bool, bool, List[str]]:
        ...
        for change in pr.changed_files:
            ...
            if change.filename.endswith(".sql") or "migrations/" in change.filename.lower():
        ...
        # 4. Check for active block review requests
        ...
        is_merge_safe = (
            ...
            all(run.conclusion == "success" for run in pr.check_runs if run.status == "completed")
        )
